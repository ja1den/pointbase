...
createLink?.addEventListener('click', event => {
	...
	// Update Title
	document.querySelector('#record-modal .modal-title').innerHTML = 'Create Sport';
...
updateLinks.forEach(element => element.addEventListener('click', event => {
	...
	// Update Title
	document.querySelector('#record-modal .modal-title').innerHTML = 'Update Sport';
	...
	inputs[1].checked = records[element.dataset.recordId][2];

	const selects = [...createUpdateForm.getElementsByTagName('select')];

	selects[0].value = parseInt(records[element.dataset.recordId][1]);
}));
