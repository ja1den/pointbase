...
router.post('/', auth, async (req, res) => {
	try {
		...
		// Create
		const { id } = await sequelize.models.sport.create(req.body);
...
router.get('/', auth, async (_req, res) => {
	try {
		// Find All
		const records = await sequelize.models.sport.findAll();
...
router.patch('/:id', auth, async (req, res) => {
	try {
		...
		// Read Record
		const record = await sequelize.models.sport.findByPk(req.params.id);
...
router.delete('/:id', auth, async (req, res) => {
	try {
		...
		// Delete
		await sequelize.models.sport.destroy({ where: { id: req.params.id } });
